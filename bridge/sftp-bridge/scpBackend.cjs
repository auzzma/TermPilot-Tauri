...
function createScpBackend(deps = {}) {
  ...
  async function downloadToWritable(remotePath, writable, {
    ...
    await new Promise((resolve, reject) => {
      ...
      const finish = (err) => {
        ...
        try { stream.__termpilotUntrack?.(); } catch { /* ignore */ }
        try { stream.__termpilotCleanupAbort?.(); } catch { /* ignore */ }
      ...
      if (stream.__termpilotEarlyError) {
        finish(stream.__termpilotEarlyError);
        return;
      }
      if (
        stream.__termpilotClosed === true
        || stream.closed === true
  ...
  function waitForAck(stream, transfer, signal = null) {
    return new Promise((resolve, reject) => {
      ...
      if (stream.__termpilotEarlyError) {
        const earlyError = stream.__termpilotEarlyError;
        cleanup();
        reject(earlyError);
        return;
      }
  ...
  function execStream(command, options = {}) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const cleanupAbort = () => {
        ...
        abortCleaned = true;
        try { signal.removeEventListener("abort", onAbort); } catch { /* ignore */ }
        if (streamRef?.__termpilotCleanupAbort === cleanupAbort) {
          delete streamRef.__termpilotCleanupAbort;
        }
      };
      const onAbort = () => {
        try { streamRef?.close?.(); } catch { /* ignore */ }
      ...
      try {
        sshClient.exec(command, (err, stream) => {
          ...
          stream.__termpilotClosed = false;
          stream.__termpilotEarlyError = null;
          stream.on("close", () => {
            stream.__termpilotClosed = true;
          });
          stream.on("error", (streamErr) => {
            stream.__termpilotEarlyError = stream.__termpilotEarlyError || streamErr;
          });
          if (signal) {
            stream.__termpilotCleanupAbort = cleanupAbort;
...
function getScpBackendForClient(client) {
  ...
  if (client.__termpilotScpBackend) return client.__termpilotScpBackend;
  ...
  if (!client.__termpilotScpActiveStreams) {
    client.__termpilotScpActiveStreams = new Set();
  }
  const base = createSshExecAdapters(sshClient);
  const track = (stream) => {
    if (!stream) return stream;
    client.__termpilotScpActiveStreams.add(stream);
    const untrack = () => {
      try { client.__termpilotScpActiveStreams.delete(stream); } catch { /* ignore */ }
    };
    stream.__termpilotUntrack = untrack;
  ...
  const trackedExec = (command, options = {}) => new Promise((resolve, reject) => {
    ...
    const cleanup = () => {
      if (streamRef) {
        try { client.__termpilotScpActiveStreams.delete(streamRef); } catch { /* ignore */ }
      }
      if (signal) {
        try { signal.removeEventListener("abort", onAbort); } catch { /* ignore */ }
      }
  ...
  client.__termpilotScpTrackedExec = trackedExec;
  const backend = createScpBackend(adapters);
  client.__termpilotScpBackend = backend;
  return backend;
}
...
function abortScpClientStreams(client) {
  const streams = client?.__termpilotScpActiveStreams;
